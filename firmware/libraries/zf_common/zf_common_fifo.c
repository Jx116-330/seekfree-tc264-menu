...
fifo_state_enum fifo_clear (fifo_struct *fifo)
{
    ...
    while(!IfxCpu_acquireMutex(&fifo->mutex));
...
//        }
    ...
    IfxCpu_releaseMutex(&fifo->mutex);
...
uint32 fifo_used (fifo_struct *fifo)
{
    ...
    boolean interrupt_state = IfxCpu_disableInterrupts();
    while(!IfxCpu_acquireMutex(&fifo->mutex));
    uint32 used = (fifo->max - fifo->size);
    IfxCpu_releaseMutex(&fifo->mutex);
    IfxCpu_restoreInterrupts(interrupt_state);
    return used;
}
...
fifo_state_enum fifo_read_buffer (fifo_struct *fifo, void *dat, uint32 *length, fifo_operation_enum flag)
{
    ...
    do
    {
        ...
        else
        {
            ...
            fifo_data_length = fifo->max - fifo->size;                                 // 获取当前数据有多少
