...
IFX_ALIGN(4) uint8  mt9v03x_image_snapshot[2][MT9V03X_H][MT9V03X_W];
volatile uint32 mt9v03x_frame_count = 0;
volatile uint8 mt9v03x_image_snapshot_index = 0;


/* CPU0 superloop owns transactions; UART ISR only fills the reply mailbox. */
typedef enum
{
    MT9V03X_RUNTIME_IDLE = 0,
    MT9V03X_RUNTIME_SCCB,
    MT9V03X_RUNTIME_ADDRESS_SENT,
    MT9V03X_RUNTIME_ADDRESS_WAIT,
    MT9V03X_RUNTIME_ACK_WAIT
} mt9v03x_runtime_state_enum;
static volatile mt9v03x_runtime_state_enum mt9v03x_runtime_state;
static mt9v03x_write_kind_enum mt9v03x_runtime_kind;
static uint8 mt9v03x_runtime_addr;
static uint16 mt9v03x_runtime_value;
static uint32 mt9v03x_runtime_since_ms;
static volatile uint8 mt9v03x_runtime_rx[3];
static volatile uint8 mt9v03x_runtime_rx_count;
...
{
    uint8 data = 0;
    if(0 == uart_query_byte(MT9V03X_COF_UART, &data)) return;
    if((0xA5 == data) && (0 == fifo_used(&camera_receiver_fifo)))
    {
...
static void mt9v03x_dma_handler(void)
{
    ...
    else
    {
        ...
        {
            uint8 next_snapshot_index = (uint8)(mt9v03x_image_snapshot_index ^ 1U);
            // 采集完成
            // 一副图像从采集开始到采集结束耗时3.8MS左右(50FPS、188*120分辨率)
            mt9v03x_dma_int_num = 0;
            mt9v03x_lost_flag   = 0;
            mt9v03x_finish_flag = 1;
            (void)memcpy(mt9v03x_image_snapshot[next_snapshot_index], mt9v03x_image, sizeof(mt9v03x_image));
            mt9v03x_image_snapshot_index = next_snapshot_index;
            ++mt9v03x_frame_count;
            dma_disable(MT9V03X_DMA_CH);
        }
    }
...
{
    if (MT9V03X_RUNTIME_IDLE != mt9v03x_runtime_state) return 0U;
    uint16 temp;
    uint8  uart_buffer[4];
    uint16 timeout_count = 0;
...
{
    if (MT9V03X_RUNTIME_IDLE != mt9v03x_runtime_state) return 1U;
    uint8 return_state = 0;
...
/* Unlike the legacy FIFO parser, 0xA5 inside the payload is ordinary data. */
static void mt9v03x_runtime_uart_handler(void)
{
    uint8 data;
    uint8 count;
    if (0U == uart_query_byte(MT9V03X_COF_UART, &data)) return;
    /* Drain late replies while idle and discard any address reply. */
    if (MT9V03X_RUNTIME_ACK_WAIT != mt9v03x_runtime_state) return;
    count = mt9v03x_runtime_rx_count;
    if ((count >= 3U) || ((0U == count) && (0xA5U != data))) return;
    mt9v03x_runtime_rx[count] = data;
    mt9v03x_runtime_rx_count = (uint8)(count + 1U);
}

static void mt9v03x_runtime_send(uint8 command, uint16 value)
{
    uint8 packet[4];
    packet[0] = 0xA5U;
    packet[1] = command;
    packet[2] = (uint8)(value >> 8);
    packet[3] = (uint8)value;
    /* Legacy TX sends four bytes (~4.2 ms at 9600 baud), without a reply wait. */
    uart_write_buffer(MT9V03X_COF_UART, packet, 4U);
}

uint8 mt9v03x_write_start(mt9v03x_write_kind_enum kind, uint8 addr,
                         uint16 value, uint32 now_ms)
{
    uint32 irq_state;
    if ((MT9V03X_RUNTIME_IDLE != mt9v03x_runtime_state) ||
        ((MT9V03X_WRITE_EXPOSURE != kind) && (MT9V03X_WRITE_REGISTER != kind)))
        return 1U;
    mt9v03x_runtime_kind = kind;
    mt9v03x_runtime_addr = addr;
    mt9v03x_runtime_value = value;
    mt9v03x_runtime_since_ms = now_ms;
    if (MT9V03X_SCCB == mt9v03x_type)
    {
        mt9v03x_runtime_state = MT9V03X_RUNTIME_SCCB;
        return 0U;
    }
    irq_state = interrupt_global_disable();
    mt9v03x_runtime_rx_count = 0U;
    mt9v03x_runtime_state = (MT9V03X_WRITE_REGISTER == kind) ?
        MT9V03X_RUNTIME_ADDRESS_SENT : MT9V03X_RUNTIME_ACK_WAIT;
    set_camera_type(CAMERA_GRAYSCALE, mt9v03x_vsync_handler,
                    mt9v03x_dma_handler, mt9v03x_runtime_uart_handler);
    interrupt_global_enable(irq_state);
    mt9v03x_runtime_send((MT9V03X_WRITE_REGISTER == kind) ?
                         MT9V03X_SET_ADDR : MT9V03X_SET_EXP_TIME,
                         (MT9V03X_WRITE_REGISTER == kind) ? addr : value);
    return 0U;
}

mt9v03x_write_status_enum mt9v03x_write_poll(uint32 now_ms)
{
    uint8 failed;
    uint16 reply;
    uint32 irq_state;
    if (MT9V03X_RUNTIME_IDLE == mt9v03x_runtime_state) return MT9V03X_WRITE_IDLE;
    if (MT9V03X_RUNTIME_SCCB == mt9v03x_runtime_state)
    {
        /* Vendor SCCB code is in zf_device_config.a: one call per transaction. */
        failed = (MT9V03X_WRITE_REGISTER == mt9v03x_runtime_kind) ?
            mt9v03x_set_reg_sccb(mt9v03x_runtime_addr, mt9v03x_runtime_value) :
            mt9v03x_set_exposure_time_sccb(mt9v03x_runtime_value);
        mt9v03x_runtime_state = MT9V03X_RUNTIME_IDLE;
        return failed ? MT9V03X_WRITE_ERROR : MT9V03X_WRITE_SUCCESS;
    }
    if (MT9V03X_RUNTIME_ADDRESS_SENT == mt9v03x_runtime_state)
    {
        /* Start the gap on the first poll AFTER TX has completed. */
        mt9v03x_runtime_since_ms = now_ms;
        mt9v03x_runtime_state = MT9V03X_RUNTIME_ADDRESS_WAIT;
        return MT9V03X_WRITE_BUSY;
    }
    if (MT9V03X_RUNTIME_ADDRESS_WAIT == mt9v03x_runtime_state)
    {
        if ((uint32)(now_ms - mt9v03x_runtime_since_ms) < 10U)
            return MT9V03X_WRITE_BUSY;
        irq_state = interrupt_global_disable();
        mt9v03x_runtime_rx_count = 0U;
        mt9v03x_runtime_state = MT9V03X_RUNTIME_ACK_WAIT;
        interrupt_global_enable(irq_state);
        mt9v03x_runtime_since_ms = now_ms;
        mt9v03x_runtime_send(MT9V03X_SET_DATA, mt9v03x_runtime_value);
        return MT9V03X_WRITE_BUSY;
    }
    if (mt9v03x_runtime_rx_count >= 3U)
    {
        /* Full mailbox is immutable until consumed; no long IRQ masking. */
        reply = (uint16)(((uint16)mt9v03x_runtime_rx[1] << 8) |
                         mt9v03x_runtime_rx[2]);
        mt9v03x_runtime_state = MT9V03X_RUNTIME_IDLE;
        return (reply == mt9v03x_runtime_value) ?
               MT9V03X_WRITE_SUCCESS : MT9V03X_WRITE_ERROR;
    }
    if ((uint32)(now_ms - mt9v03x_runtime_since_ms) >= (MT9V03X_INIT_TIMEOUT + 5U))
    {
        mt9v03x_runtime_state = MT9V03X_RUNTIME_IDLE;
        return MT9V03X_WRITE_ERROR;
    }
    return MT9V03X_WRITE_BUSY;
}
