...
static uint8 gnss_checksum_valid (const uint8 *buffer)
{
    uint32 i;
    uint8 checksum = 0;
    uint8 check_text[5] = { '0', 'x', 0, 0, 0 };
    if((NULL == buffer) || (0x24 != buffer[0]) || (0 == buffer[1])) return 0;
    for(i = 2; (i < (GNSS_BUFFER_SIZE - 3)) && (0x2A != buffer[i]); i ++) checksum ^= buffer[i];
    if((0x2A != buffer[i]) || (0 == buffer[i + 1]) || (0 == buffer[i + 2])) return 0;
    check_text[2] = buffer[i + 1];
    check_text[3] = buffer[i + 2];
    return checksum == (uint8)func_str_to_hex((char *)check_text);
}
...
uint8 gnss_data_parse (void)
{
    ...
    do
    {
        if(GPS_STATE_RECEIVED == gnss_rmc_state)
        {
            ...
            if(!gnss_checksum_valid(gps_rmc_buffer)) { gnss_rmc_state = GPS_STATE_RECEIVING; return_state = 1; break; }
        ...
        if(GPS_STATE_RECEIVED == gnss_gga_state)
        {
            ...
            if(!gnss_checksum_valid(gps_gga_buffer)) { gnss_gga_state = GPS_STATE_RECEIVING; return_state = 1; break; }
        ...
        if(GPS_STATE_RECEIVED == gnss_ths_state)
        {
            ...
            if(!gnss_checksum_valid(gps_ths_buffer)) { gnss_ths_state = GPS_STATE_RECEIVING; return_state = 1; break; }
...
void gnss_uart_callback (void)
{
    ...
    if(gnss_state)
    {
        ...
        if(received && ('\n' == dat))
        {
            ...
            if(6 > temp_length) { fifo_clear(&gnss_receiver_fifo); return; }
            ...
            if(0 == strncmp((char *)&temp_gps[3], "RMC", 3))
            {
                ...
                if(GPS_STATE_PARSING != gnss_rmc_state)
                {
                    ...
                    if(GNSS_BUFFER_SIZE <= temp_length) temp_length = GNSS_BUFFER_SIZE - 1;
